...
public class ProductsAdminDao {
    ...
    public List<Product> findProductsByFilter(Filter filter) {
        ...
        Predicate predicateResultAttr = makePredicateByFilter(cb, product,
                            predicatesAllAttributes, filter.getLanguage(), filter.getCode(),
                filter.getName(), filter.getCategory(), filter.getManufacturer(),
                filter.getAmount(), filter.getPriceFrom(), filter.getPriceTo());
        ...
        boolean useFilters = predicateResultAttr != null && predicatesAllAttributes.size() > 0;
        Join<Product, ProductAttribute> attributeSort = product.join("attributes", JoinType.LEFT);
        attributeSort.on(
                cb.equal(attributeSort.get("name"), filter.getName())
        );

        final String SORT = filter.getSort();


        if (SORT.equals(Constants.PRODUCT_ATTR_PRICE_FROM) || SORT.equals(Constants.PRODUCT_ATTR_PRICE_TO)
                || SORT.equals(Constants.PRODUCT_ATTR_AMOUNT)) {
            sortType = attributeSort.get("value").as(Integer.class);
        } else {
            sortType = attributeSort.get("value").as(String.class);
        }
        Order orderSort;
        if (filter.getOrder().equals(Constants.SORT_VECTOR_DESC)) {
        ...
        TypedQuery<Product> query = em.createQuery(cq).setFirstResult(filter.getLimit() * Math.max(filter.getPage() -1, 0)).setMaxResults(filter.getLimit());
    ...
    public List<Product> findProductsIdsByFilter(String language,
        ...
        //set filters count - количество, которое выражает все варианты продуктов которые мы находим, подходих вариантов продуктов должно быть равное произведению всех фильтров
        int filterCount = Math.max(predicatesAllAttributes.size() - 1, 0);

        if (predicatesAllAttributes.size() > 0 && predicateResultAttr != null) { //if filter used
            cq.where(predicateResultAttr).having(cb.gt(cb.count(product.get("id")), filterCount));
        }
        return query.getResultList();
    }
